    ...
    def _generate_internal_minutes(
        ...
        *,
        ...
        prompt = (
            ...
            "ASR转写稿是不可修改的原始底稿，其中的同音字、近音字和专名错误是正常噪声，不要回写或要求先纠正ASR。"
            "生成纪要时，专名优先采用用户确认信息、工作背景和补充材料中的明确写法；"
            "若多个来源冲突，不要凭猜测选择，以中性称谓概括并在内部留档版标出待校对位置。"
    ...
    def _generate_work_minutes(
        ...
        *,
        ...
        prompt = (
            ...
            "不要把ASR中的同音字直接复制到正式纪要；专名必须与用户确认信息、工作背景或附件材料交叉核对，"
            "无法确认时使用中性称谓，不要擅自创造或替换名称。"
