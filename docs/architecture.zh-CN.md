...
当前文档描述现有代码的真实边界。面向任意代码隔离、权限、工作区快照、宿主能力、会议 ASR 接入、状态恢复和交付验收的目标态设计，见 [《Work Agent 安全执行平台开发设计》](secure-execution-platform.zh-CN.md)。该设计要求执行后端不可用时失败关闭，不允许静默回退到宿主执行。
